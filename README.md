...
- Logs raw request and response pairs as JSONL under `~/.mddlmn/logs/`.
- Stores sessions, requests, sections, token counts, and durations in `~/.mddlmn/data/mddlmn.sqlite`.
...
Runtime data lives outside the repo under `~/.mddlmn/` (`data/mddlmn.sqlite`, `logs/`, plus redaction/injection/memory config). Override the location with `MDDLMN_CONFIG_DIR`.
...
code --install-extension mddlmn-0.1.3.vsix
```
...
Runtime state lives under `~/.mddlmn/`: `data/mddlmn.sqlite`, `logs/*.jsonl`, and the redaction/injection/memory config files. Override the directory with `MDDLMN_CONFIG_DIR`. Session-scoped memory entries are never written to disk — they live only in RAM and are gone on proxy restart.

Generated artifacts under the repo (also gitignored):
...
mddlmn is a local developer tool. It records full prompts, tool results, and model responses under `~/.mddlmn/logs/` and `~/.mddlmn/data/`, so treat those files as sensitive.
